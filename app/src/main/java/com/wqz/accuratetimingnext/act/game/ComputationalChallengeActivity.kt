 ...
 */
@AndroidEntryPoint
class ComputationalChallengeActivity : ComponentActivity() {
    ...
    fun ChallengeScreen(
                ...
                if (actualTimes.size == expectedTimes.size && errorTimes.size == expectedTimes.size) {
                    ...
                    gameRecordViewModel.insert(
                        gameRecord = GameRecord(
                            ...
                            gameTag = ModeTranslator.getChineseName(mode = Mode.COMPUTATIONAL_MODE)
                        )
                    )
        ...
        // 生成随机计算式（记住当前索引的表达式）
        val expression = remember(currentTargetIndex) {
            if (currentTargetIndex < expectedTimes.size) {
                generateRandomExpression(expectedTimes[currentTargetIndex])
            } else {
                ""
            }
        }
        ...
        XCard.Lively {
            ...
            Text(
                ...
                modifier = Modifier.clickVfx {
                    val intent =
                        Intent(
                            this@ComputationalChallengeActivity,
                            ErrorDetailsActivity::class.java
                        )
                    val bundle = Bundle()
                    bundle.putLongArray(
                        "EXPECTED_TIMES",
    ...
    /**
     * 生成随机计算表达式
     * @param targetMillis 目标时间（毫秒）
     * @return 计算表达式字符串，如 "7.85 - 1.19"
     */
    private fun generateRandomExpression(targetMillis: Long): String {
        val random = Random
        val targetSeconds = targetMillis.toFloat() / 1000  // 转换为秒

        return if (random.nextBoolean()) {
            // 生成加法表达式: a + b = targetSeconds
            val a =
                (random.nextFloat() * (targetSeconds - 0.1f)) + 0.1f  // 随机加数 1 (0.1 到 targetSeconds-0.1 之间)
            val b = targetSeconds - a  // 计算加数 2
            "${"%.2f".format(a)} + ${"%.2f".format(b)}"
        } else {
            // 生成减法表达式: a - b = targetSeconds
            val b =
                (random.nextFloat() * (targetSeconds - 0.1f)) + 0.1f  // 随机减数 (0.1 到 targetSeconds-0.1 之间)
            val a = targetSeconds + b  // 计算被减数
            "${"%.2f".format(a)} - ${"%.2f".format(b)}"
        }
    }
}