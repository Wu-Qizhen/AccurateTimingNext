 ...
 */
@AndroidEntryPoint
class RaceRecordActivity : ComponentActivity() {
        ...
        LaunchedEffect(gameFinished) {
            if (gameFinished && !recordSaved && player != null && time != null) {
                ...
                if (actualTimes.size == expectedTimes.size && errorTimes.size == expectedTimes.size) {
                    /*val competitionRound = player!!.numberOfCompetitions + 1
                    val accumulationNumber = player!!.accumulationNumber + actualTimes.size
                    val accumulationError = player!!.accumulationError + totalError
                    val accumulationTime = player!!.accumulationTime + actualTimes.sum()
                    val bestRecords = player!!.bestRecords
                    val maximumAverageErrors = player!!.maximumAverageErrors
                    val minimumAverageErrors = player!!.minimumAverageErrors*/

                    /*val index = expectedTimes.size - 1
                    var competitionRound: Int
                    var accumulationNumber: Int
                    var accumulationError: Long
                    var accumulationTime: Long
                    var bestRecords: IntArray
                    var maximumAverageErrors: IntArray
                    var minimumAverageErrors: IntArray
                    val averageError = totalError / actualTimes.size

                    player?.let {
                        competitionRound = it.numberOfCompetitions + 1
                        accumulationNumber = it.accumulationNumber + actualTimes.size
                        accumulationError = it.accumulationError + totalError
                        accumulationTime = it.accumulationTime + actualTimes.sum()
                        bestRecords = it.bestRecords
                        maximumAverageErrors = it.maximumAverageErrors
                        minimumAverageErrors = it.minimumAverageErrors
                    } ?: run {
                        XToast.showText("加载失败")
                    }

                    if (bestRecords[index] > totalError) {
                        bestRecords[index] = totalError
                    }

                    if (maximumAverageErrors[index] < averageError]< averageError){
                        maximumAverageErrors[index] = averageError
                    }

                    if (minimumAverageErrors[index] > averageError]> averageError){
                        minimumAverageErrors[index] = averageError
                    }
                    ...
                    playerViewModel.update(
                        player = Player(
                            ...
                            numberOfCompetitions = competitionRound,
                            accumulationNumber = accumulationNumber,
                            accumulationError = accumulationError,
                    ...
                    player?.let { currentPlayer ->
                        ...
                        /*// 更新数组值（安全索引访问）
                        if (index in bestRecords.indices && totalError < bestRecords[index]) {
                            bestRecords[index] = totalError.toInt()
                        }
                        if (index in maximumAverageErrors.indices && averageError > maximumAverageErrors[index]) {
                            maximumAverageErrors[index] = averageError.toInt()
                        }
                        if (index in minimumAverageErrors.indices && averageError < minimumAverageErrors[index]) {
