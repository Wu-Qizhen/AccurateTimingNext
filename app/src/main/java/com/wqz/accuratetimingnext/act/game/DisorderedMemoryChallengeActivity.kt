 ...
 */
@AndroidEntryPoint
class DisorderedMemoryChallengeActivity : ComponentActivity() {
    ...
    fun MemoryScreen(expectedTimes: List<Long>, onSkip: () -> Unit) {
        XCard.LivelyCard(10) {
            ...
            Text(
                text = expectedTimes.shuffled().joinToString("\n") { "${it.toDouble() / 1000}S" },
                fontSize = 16.sp,
    ...
    fun ChallengeScreen(
        ...
        // 添加记忆屏幕状态
        var showMemoryScreen by remember { mutableStateOf(true) }

        // 10 秒后自动切换到挑战界面
        LaunchedEffect(showMemoryScreen) {
            if (showMemoryScreen) {
                delay(10000)  // 10秒延迟
                ...
                if (actualTimes.size == expectedTimes.size && errorTimes.size == expectedTimes.size) {
                    ...
                    gameRecordViewModel.insert(
                        gameRecord = GameRecord(
                            ...
                            gameTag = ModeTranslator.getChineseName(mode = Mode.DISORDERED_MEMORY_MODE)
                        )
                    )
        ...
        XCard.LivelyCard {
            ...
            Text(
                ...
                modifier = Modifier.clickVfx {
                    val intent =
                        Intent(
                            this@DisorderedMemoryChallengeActivity,
